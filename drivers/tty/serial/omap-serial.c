...
static inline void serial_omap_update_gpio_rts(struct uart_omap_port *up, unsigned int value)
{
/*
...
*/
	gpio_direction_output(up->rts_gpio, value);
}
...
static inline void serial_omap_update_rts(struct uart_omap_port *up)
{
	...
	printk("===== %s: start mcr = %u =====\n", __func__, mcr);
	printk("===== %s: start rts_on_send = %u =====\n", __func__, rts_on_send);
	printk("===== %s: start rs485.flags = %u =====\n", __func__, up->rs485.flags);
...
//			serial_omap_update_rts(up);
		...
		printk("RS-485 start tx\n");
		serial_omap_update_gpio_rts(up, 1);
	}
...
static inline irqreturn_t serial_omap_irq(int irq, void *dev_id)
{
	...
	if (up->tx_wait_end && (iir & UART_IIR_THRI) &&
		...
		serial_omap_disable_ier_thri(up);
//		serial_omap_update_rts(up);
		serial_omap_update_gpio_rts(up, 0);
		printk("RS-485 finish tx\n");
		goto handled;
	}
	lsr = serial_in(up, UART_LSR);
...
serial_omap_ioctl(struct uart_port *port, unsigned int cmd, unsigned long arg)
{
	...
	case TIOCSRS485:
		...
		if (!(rs485conf.flags & SER_RS485_ENABLED)) {
			printk("===== %s: !SER_RS485_ENABLED =====/n",__func__);
			up->tx_in_progress = 0;
			up->tx_wait_end = 0;
		}
		up->rs485 = rs485conf;
